// THis file was not written by the author of the BadassVishnu program, the following comments are retained from the original authorsm the only part changed is the package name
...
/**
 * A dead simple command-line argument parser. Because I couldn't find any one
 * in Java that wasn't horribly bloated.
 * <p>
 * Example:
 * <pre>{@literal
 ...
 *     ...
 *   }
 * }</pre> This parser honors the convention that argument {@code --} means
...
public final class ArgP
{
    ...
    /**
     * Maps an option name to the value parsed for this option. The value can be
     * {@code null}.
     */
	...
	final String[] prev = options.put(name, new String[]
	{
	    meta, help
	});
	if (prev != null)
    ...
    /**
     * Returns the value of the given option, if it was given. Returns
     * {@code null} if the option wasn't given, or if the option doesn't take a
     * value (in which case you should use {@link #has} instead).
     *
     * @param name The name of the option to recognize (e.g. {@code --foo}).
